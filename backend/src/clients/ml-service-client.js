...
class MlServiceClient {
  ...
  async executeRequest({ attempt, body, method, path, requestId, timeoutMs }) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          // An invalid error payload is represented by the stable fallback below.
        }
        logger.warn(
          {
            attempt,
        ...
        return {
          data: null,
          error: {
            code: upstreamError?.error?.code ?? 'ML_SERVICE_HTTP_ERROR',
            message: upstreamError?.error?.message ?? 'ML service returned an unsuccessful response.',
          },
          ok: false,
          responseTimeMs,
