...
flowchart TB
  ...
  MLS["Python ML Microservice\nFastAPI, readiness, metadata, integrity checks"]
...
The browser has no direct model-service access. The backend is the public trust boundary. The Python service owns versioned inference package loading; it does not train or select models. Governance artifacts bind the package to its frozen dataset, configuration, and experiment evidence. The package's current status is integration-only, not deployment-approved.
